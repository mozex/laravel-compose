...
- Docker Engine with the Compose plugin, so `docker compose version` works. Ploi's Docker server type and Forge's Docker install both provide it; on a plain Ubuntu box, follow Docker's own install guide and add `docker-compose-plugin`.
